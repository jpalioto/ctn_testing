...
def main():
    ...
    parser.add_argument(
        "--config", type=Path, required=True, help="Path to config (for judge model)"
    )
                ...
                try:
                    ...
                    if judge_result.fields:
                        result["composite_score"] = sum(
                            f.composite_score for f in judge_result.fields
                        ) / len(judge_result.fields)
                        result["value_score"] = sum(
                            f.value_score for f in judge_result.fields
                        ) / len(judge_result.fields)
