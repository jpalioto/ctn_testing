...
from ctn_testing.core.types import Candidate, Evidence, Extraction, ExtractionStatus
from ctn_testing.metrics.scorer import composite_score


def make_ext(
    field_name="total", value=None, quote=None, page=None, status="ok", candidates=None
) -> Extraction:
    return Extraction(
...
def make_gt(
    field_name="total", exists=True, ambiguous=False, value=None, candidate_values=None
) -> GroundTruth:
    return GroundTruth(
        field_name=field_name,
