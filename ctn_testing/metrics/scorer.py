...
from ..core.types import Candidate, CompositeResult, Extraction, ExtractionStatus
...
def evidence_validity(ext: Extraction, gt: GroundTruth, document_text: str, judge_fn=None) -> float:
    """
...
def candidate_evidence_score(
    candidates: list[Candidate], document_text: str, judge_fn=None
) -> float:
...
def composite_score(
    ext: Extraction, gt: GroundTruth, document_text: str, pages: list[str], judge_fn=None
) -> CompositeResult:
    """
    ...
    """
    weights = {"value": 0.30, "evidence": 0.30, "page": 0.10, "status": 0.15, "schema": 0.15}
    ...
    return CompositeResult(
        composite=composite, value=val, evidence=evidence, page=page, status=status, schema=schema
    )
